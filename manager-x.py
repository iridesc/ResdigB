...
class Task:
    def __init__(self, keyword, subtaskqueue):
        ...
        self.reslist = []
        subtaskqueue.put(
            SubTask(task_type="ParseTask", keyword=keyword, DEEPTH=DEEPTH,)
        )
    ...
    def do(self):
            ...
            def get_rawres(sourcecode):
                ...
                for res_container in [
                    ...
                    for reslink in res_container[0]:
                        if len(reslink) < 800:
                            raw_res = RawRes(
                                self.keyword, reslink, self.link, res_container[1]
                            )
                            # 补全信息
        ...
        def parsetask():
            ...
            CACHE.subtaskqueue_puts(
                self.keyword,
                [
                    SubTask(task_type="MiniTask", keyword=self.keyword, weblink=weblink)
...
class Cache:
    ...
    def saveRes(self):
        for task in self.tasks:
            if task.statu == "Done" or (
                task.statu == "Digging" and time.time() - task.last_active_time > 20
            ):
                # 除重
                savereslist = []
                prelinklist = [
...
if __name__ == "__main__":
    while True:
        ...
        try:
            # 启动服务
            cache = initManager(
                isManager=True, obj=Cache(), port=port, password=password
            )
            ...
            makelog(
                "Error in main process! Reboot after 2s !\n{}".format(
